...
try {
    ...
    if ($Clean) {
        foreach ($project in @($contractProject, $hostProject, $payloadProject)) {
            Invoke-DotNet (@("restore", $project) + $properties + $verbosity)
        }
    ...
    if (-not $Quiet) {
        $host_path = Join-Path $projectRoot "bin\rh$RhinoMajorVersion\$Configuration\$TargetFramework\RhinoViterRuntimeScripts.rhp"
        ...
        Write-Host "Built host: $host_path"
