...
$devInstallRoot = [IO.Path]::GetFullPath((Join-Path $projectRoot 'bin\RhinoViterRuntimeScriptsDev'))
if ([string]::IsNullOrWhiteSpace($InstallDirectory)) {
    $InstallDirectory = Join-Path $devInstallRoot "rh$RhinoMajorVersion"
}
$installDirectory = [IO.Path]::GetFullPath($InstallDirectory)
$installPrefix = $devInstallRoot.TrimEnd('\', '/') + [IO.Path]::DirectorySeparatorChar
if (-not $installDirectory.StartsWith($installPrefix, [StringComparison]::OrdinalIgnoreCase)) {
    throw "Refusing runtime installation outside '$devInstallRoot'."
}
