...
let mutable before_run_callback = fun () -> ()
let alias_macro_prefix = "! _-RuntimeScriptsRun "

let set_before_run (callback: unit -> unit) =
    lock gate (fun () -> before_run_callback <- callback)

let prepare_run () =
    let callback = lock gate (fun () -> before_run_callback)
    callback ()
...
let alias_macro (name: string) = $"{alias_macro_prefix}{name}"

let owned_alias (name: string) =
    CommandAliasList.IsAlias name
    && CommandAliasList.GetMacro(name).StartsWith(alias_macro_prefix, StringComparison.OrdinalIgnoreCase)
...
let validate_aliases (incoming: RuntimeCommandDefinition array) =
    ...
    |> Array.tryPick (fun (definition: RuntimeCommandDefinition) ->
        if CommandAliasList.IsAlias definition.name then
            ...
            let command_id = Command.LookupCommandId(definition.name, true)

            if command_id = Guid.Empty then
                None
            else
                Some $"Rhino already has a command named {definition.name} with GUID {command_id}.")
...
let replace (incoming: RuntimeCommandDefinition array) =
    ...
    | Ok valid_definitions ->
        try
            match sync_aliases valid_definitions with
            | Error message -> Error message
            | Ok() ->
                lock gate (fun () ->
                    let replacement =
                    ...
                    for definition in valid_definitions do
                        replacement.Add(definition.name, definition)

                    definitions <- replacement)

                Ok valid_definitions.Length
        with error ->
